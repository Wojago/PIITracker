...
function getListProperties() {
    ...
    function onQuerySucceeded(sender, args) {
        ...
        if (currentItem.get_item('Type') == 'List') {
            var options1 = SP.UI.$create_DialogOptions(); options1.width = 600;
            options1.height = 100;
        ...
        if (currentItem.get_item('Type') == 'Item') {
            ...
            options.resizable = 1;
            options.scroll = 1;
            options.url = '/_layouts/DCMA.SP.TrackingPII/RemovePerms.aspx?itemId=' + currentItem.get_item('itemID') + '&plName=' + currentItem.get_item('Parent') + '&type=' + currentItem.get_item('Type') + '&webUrl=' + currentItem.get_item('SiteURL') + '&pURL=' + siteUrl + '&cID=' + selId;

            SP.UI.ModalDialog.showModalDialog(options);
        }
        // currentItem.set_item('Status', 'Permission is broken');
    }
